...
export default function Home() {
  ...
  const { data, error, loading } = useQuery(AllPERSON)
  ...
  <section className="bg-gray-50 dark:bg-gray-900 p-3 sm:p-5">
    <div className="mx-auto max-w-screen-xl px-4 lg:px-12">
                          ...
                          data?.allPerson.map((i) => {
                            return (
                              <tr key={i.id} className="border-b dark:border-gray-700">
                                <th scope="row" className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white"> {i.id} </th>
                                <td className="px-4 py-3">{i.nombre}</td>
                                <td className="px-4 py-3">{i.telefono}</td>
                                <td className="px-4 py-3">{i.ciudad}</td>
                            </tr>
                            )
                          })
                        }
      ...
      <div id="updateProductModal" tabindex="-1" aria-hidden="true" className=" overflow-y-auto overflow-x-hidden top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
